...
- Run the full test suite (real loopback sockets, no test framework/dependency): `python test_framework.py`
- Sanity-check imports: `python -c "import connections; print('import ok')"`
- Compile-check the whole package: `python -m compileall -q connections`

Run the test suite from the **repo root** (`C:\Projects\rapay`), not from this directory —
`connections` now imports the sibling `IRS` and `tools` packages, so the root has to be on
`sys.path`.
